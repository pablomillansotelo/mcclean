...
import electron, { app as app$1, BrowserWindow, ipcMain as ipcMain$1, dialog, shell as shell$1, Menu } from "electron";
...
async function getPathSize(itemPath, isDir) {
  if (!isDir) {
    const stats = await fs$1.stat(itemPath);
    return stats.size;
  }
  try {
    const { stdout } = await execAsync(`du -sk "${itemPath}"`);
    const sizeInK = parseInt(stdout.split("	")[0]);
    return sizeInK * 1024;
  } catch (e) {
    return 0;
  }
}
async function scanDirectory(event, targetPath) {
  try {
    const homeDir = os.homedir();
    const startPath = targetPath ? path.resolve(targetPath) : homeDir;
    event.sender.send("scan-progress", {
      scanId: "custom",
      progress: 0,
      status: `Scanning ${startPath}...`
    });
    let allResults = [];
    try {
      const entries = await fs$1.readdir(startPath, { withFileTypes: true });
      const dirResults = await Promise.all(
        entries.map(async (entry) => {
          ...
          try {
            ...
            const size = await getPathSize(fullPath, isDir);
            return {
              name: entry.name,
              path: fullPath,
              size,
      ...
      allResults = dirResults.filter((r) => r !== null);
    } catch (err) {
      console.error(`Error scanning ${startPath}:`, err);
      return [];
    }
    return allResults.sort((a, b) => b.size - a.size);
  } catch (error2) {
...
function isSafeToDelete(filePath) {
  const safePaths = [path.join(os.homedir(), "Library/Caches"), path.join(os.homedir(), "Library/Logs"), "/Library/Caches", "/Library/Logs"];
  const normalized = path.resolve(filePath);
  return safePaths.some((safe) => normalized.startsWith(safe) && normalized !== safe && normalized.length > safe.length);
}
async function scanSystem(event) {
  const systemPaths = [
    { name: "System Caches", path: "/Library/Caches", type: "System Cache" },
    { name: "System Logs", path: "/Library/Logs", type: "System Logs" },
    { name: "User Logs", path: path.join(os.homedir(), "Library/Logs"), type: "User Logs" }
  ];
  const results = [];
  let processed = 0;
  for (const item of systemPaths) {
    event.sender.send("scan-progress", {
      scanId: "system",
      progress: processed / systemPaths.length * 100,
      status: `Analyzing ${item.name}...`
    });
    try {
      const entries = await fs$1.readdir(item.path, { withFileTypes: true });
      ...
      console.log(`Access denied or error for ${item.path}`, e);
    }
    processed++;
  }
  event.sender.send("scan-progress", { scanId: "system", progress: 100, status: "Done" });
  return results.sort((a, b) => b.size - a.size);
}
async function scanApplications(event) {
  try {
    ...
    for (const dir of appDirs) {
      ...
      try {
        const stats = await fs$1.stat(dir).catch(() => null);
        if (!stats) continue;
        const entries = await fs$1.readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
          if (entry.name.endsWith(".app")) {
            const fullPath = path.join(dir, entry.name);
            const size = await getPathSize(fullPath, true);
            apps.push({
    ...
    event.sender.send("scan-progress", { scanId: "apps", progress: 100, status: "Done" });
...
async function scanBrew(event) {
  try {
    ...
    event.sender.send("scan-progress", { scanId: "brew", progress: 10, status: "Checking Formulae..." });
    const { stdout: formulaeOut } = await execAsync("brew list --formulae --versions");
    event.sender.send("scan-progress", { scanId: "brew", progress: 50, status: "Checking Casks..." });
    const { stdout: casksOut } = await execAsync("brew list --casks --versions");
    event.sender.send("scan-progress", { scanId: "brew", progress: 90, status: "Checking Leaves..." });
    const { stdout: leavesOut } = await execAsync("brew leaves");
    const leaves = new Set(leavesOut.trim().split("\n"));
    event.sender.send("scan-progress", { scanId: "brew", progress: 100, status: "Done" });
    const parseBrewOutput = (output, type2) => {
    ...
    return [...parseBrewOutput(formulaeOut, "Formula"), ...parseBrewOutput(casksOut, "Cask")];
  } catch (e) {
    console.error("Brew scan error", e);
    return [];
  }
}
async function scanDevTools(event) {
  try {
    event.sender.send("scan-progress", { scanId: "devtools", progress: 0, status: "Initializing..." });
    const homeDir = os.homedir();
    const projectDirs = [
      ...
      homeDir
      // Fallback but might be slow if we go too deep
    ];
    const targetDir = projectDirs.find(async (d) => {
      try {
        await fs$1.stat(d);
        return true;
      } catch {
    ...
    event.sender.send("scan-progress", { scanId: "devtools", progress: 20, status: "Searching node_modules..." });
    const { stdout: nodeStr } = await execAsync(`find "${targetDir}" -name "node_modules" -type d -prune -maxdepth 5`);
    const nodePaths = nodeStr.trim().split("\n").filter(Boolean);
    const devResults = await Promise.all(
      nodePaths.map(async (p, idx) => {
        if (idx % 5 === 0) {
          event.sender.send("scan-progress", { scanId: "devtools", progress: 20 + idx / nodePaths.length * 30, status: "Analyzing Node projects..." });
        }
        const size = await getPathSize(p, true);
        return {
          name: "node_modules",
          path: p,
          size,
    ...
    event.sender.send("scan-progress", { scanId: "devtools", progress: 50, status: "Searching Python venvs..." });
    const { stdout: venvStr } = await execAsync(`find "${targetDir}" -name "pyvenv.cfg" -maxdepth 5`);
    const venvFiles = venvStr.trim().split("\n").filter(Boolean);
    const venvResults = await Promise.all(
      venvFiles.map(async (f, idx) => {
        if (idx % 5 === 0) {
          event.sender.send("scan-progress", { scanId: "devtools", progress: 50 + idx / venvFiles.length * 40, status: "Analyzing Python projects..." });
        }
        const venvPath = path.dirname(f);
        const size = await getPathSize(venvPath, true);
        return {
    ...
    event.sender.send("scan-progress", { scanId: "devtools", progress: 65, status: "Checking Xcode DerivedData..." });
    const xcodePath = path.join(homeDir, "Library/Developer/Xcode/DerivedData");
    const xcodeSize = await getPathSize(xcodePath, true);
    const xcodeResult = xcodeSize > 0 ? [
      {
    ...
    event.sender.send("scan-progress", { scanId: "devtools", progress: 75, status: "Checking Docker..." });
    const dockerPath = path.join(homeDir, "Library/Containers/com.docker.docker/Data/vms/0/data/Docker.raw");
    let dockerResult = [];
    try {
      const stats = await fs$1.stat(dockerPath);
      dockerResult = [
        {
    ...
    event.sender.send("scan-progress", { scanId: "devtools", progress: 85, status: "Checking Package Caches..." });
    const cachePaths = [
      { name: "npm Cache", path: path.join(homeDir, ".npm"), type: "npm" },
      { name: "Yarn Cache", path: path.join(homeDir, "Library/Caches/Yarn"), type: "Yarn" },
      { name: "pnpm Store", path: path.join(homeDir, "Library/pnpm/store"), type: "pnpm" },
      { name: "CocoaPods Cache", path: path.join(homeDir, "Library/Caches/CocoaPods"), type: "CocoaPods" }
    ];
    const cacheResults = await Promise.all(
      cachePaths.map(async (c) => {
        const size = await getPathSize(c.path, true);
        if (size > 0) {
          return {
            name: c.name,
            path: c.path,
    ...
    event.sender.send("scan-progress", { scanId: "devtools", progress: 100, status: "Done" });
    return [...devResults, ...venvResults, ...xcodeResult, ...dockerResult, ...cacheResults.filter((r) => r !== null)].sort((a, b) => b.size - a.size);
  } catch (e) {
    console.error(e);
    event.sender.send("scan-progress", { scanId: "devtools", progress: 100, status: "Error" });
    return [];
  }
}
function createWindow() {
  win = new BrowserWindow({
    ...
    // or 'sidebar', 'header', 'menu'
    visualEffectState: "active",
    titleBarStyle: "hiddenInset",
    // 'hidden' or 'hiddenInset' for traffic lights
    trafficLightPosition: { x: 12, y: 12 },
    transparent: true,
    backgroundColor: "#00000000",
    // standard transparent hex
    webPreferences: {
  ...
  ipcMain$1.handle("scan-directory", scanDirectory);
  ipcMain$1.handle("scan-system", scanSystem);
  ipcMain$1.handle("scan-apps", scanApplications);
  ipcMain$1.handle("scan-brew", scanBrew);
  ipcMain$1.handle("scan-devtools", scanDevTools);
  ipcMain$1.handle("run-brew-update", async () => {
    try {
      await execAsync("brew update && brew upgrade");
      return true;
    } catch (e) {
      console.error("Brew update error", e);
      return false;
    }
  });
  ipcMain$1.handle("find-associated-files", async (_event, appName) => {
    try {
      const { stdout } = await execAsync(`mdfind -name "${appName}"`);
      const files = stdout.split("\n").filter((f) => f && !f.includes(".app") && (f.includes("Library") || f.includes("Application Support")));
      return files.slice(0, 20);
    } catch {
      return [];
    }
  });
  ipcMain$1.handle("run-brew-uninstall", async (_event, packageName) => {
    const { response } = await dialog.showMessageBox(win, {
      ...
      detail: "This action currently cannot be undone."
    });
    if (response === 1) {
      try {
        await execAsync(`brew uninstall ${packageName}`);
        return true;
      } catch {
        return false;
      }
  ...
  ipcMain$1.handle("move-to-trash", async (_event, filePath) => {
    if (filePath.startsWith("/System") || filePath.startsWith("/Library") || filePath.includes("/Library/")) {
      if (!isSafeToDelete(filePath)) {
        dialog.showErrorBox("Restricted Access", `This item is protected by McClean safety rules to prevent system damage.
...
Path: ${filePath}`);
    ...
    const { response } = await dialog.showMessageBox(win, {
      ...
      message: `Are you sure you want to move ${path.basename(filePath)} to Trash?`,
      detail: "You can restore it later from the Trash."
    });
    if (response === 1) {
      try {
        await shell$1.trashItem(filePath);
        return true;
      } catch (e) {
  ...
  ipcMain$1.handle("scan-startup-items", async () => {
    const homeDir = os.homedir();
    try {
      const agentsPath = path.join(homeDir, "Library/LaunchAgents");
      const { stdout } = await execAsync(`ls -la "${agentsPath}"`);
      const items2 = stdout.trim().split("\n").slice(3).map((line) => {
        const parts = line.split(/\s+/);
        const name = parts[parts.length - 1];
        return name;
      }).filter((name) => name && name !== "." && name !== "..");
      return items2.map((name) => ({
        name,
        path: path.join(agentsPath, name),
        type: "LaunchAgent",
        enabled: true
        // placeholder
      }));
  ...
  ipcMain$1.handle("get-system-stats", async () => {
  ...
  ipcMain$1.handle("scan-privacy", async () => {
    const homeDir = os.homedir();
    const browsers = [
      { name: "Google Chrome", path: path.join(homeDir, "Library/Caches/Google/Chrome"), type: "Browser Cache" },
      { name: "Firefox", path: path.join(homeDir, "Library/Caches/Firefox"), type: "Browser Cache" },
      { name: "Safari", path: path.join(homeDir, "Library/Caches/com.apple.Safari"), type: "Browser Cache" }
    ];
    const results = await Promise.all(
      browsers.map(async (b) => {
        try {
          const size = await getPathSize(b.path, true);
          return { ...b, size };
        } catch {
          return { ...b, size: 0 };
        }
  ...
  ipcMain$1.handle("clean-privacy", async (_event, path2) => {
    try {
      await shell$1.trashItem(path2);
      return true;
    } catch (e) {
      console.error("Failed to trash privacy item", e);
      return false;
    }
  });
  ipcMain$1.handle("open-security-settings", async () => {
    await execAsync("open 'x-apple.systempreferences:com.apple.preference.security?Privacy'");
    return true;
  });
  ipcMain$1.handle("scan-space-lens", async () => {
    const homeDir = os.homedir();
    try {
      const { stdout } = await execAsync(`ls -d "${homeDir}"/*/`);
      const folders = stdout.trim().split("\n");
      const results = await Promise.all(
        folders.map(async (folder) => {
          const name = path.basename(folder);
          const size = await getPathSize(folder, true);
          return {
            name,
            path: folder,
            size,
      ...
      console.error("Space Lens scan error", e);
      return [];
    }
  });
  ipcMain$1.handle("get-trash-size", async () => {
    const homeDir = os.homedir();
    try {
      return await getPathSize(path.join(homeDir, ".Trash"), true);
    } catch {
      return 0;
    }
  });
  ipcMain$1.handle("scan-duplicates", async (event, targetPath = os.homedir()) => {
    try {
      event.sender.send("scan-progress", { scanId: "duplicates", progress: 0, status: "Indexing files..." });
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
              if (["Library", "node_modules", ".git"].includes(entry.name)) continue;
      ...
      await scanRecursive(targetPath);
      const potentialDuplicates = [];
      for (const [, paths] of sizeMap) {
        if (paths.length > 1) {
          potentialDuplicates.push(...paths);
        }
      }
      event.sender.send("scan-progress", { scanId: "duplicates", progress: 30, status: `Analyzing ${potentialDuplicates.length} potential duplicates...` });
      ...
      for (const filePath of potentialDuplicates) {
        ...
        if (processed % 50 === 0) {
          event.sender.send("scan-progress", {
            scanId: "duplicates",
            progress: 30 + processed / potentialDuplicates.length * 40,
            status: `Hashing... ${processed}/${potentialDuplicates.length}`
          });
        }
        try {
      ...
      for (const [, paths] of hashMap) {
        if (paths.length > 1) {
          fullHashCandidates.push(paths);
        }
      }
      const finalGroups = [];
      let groupIdx = 0;
      for (const paths of fullHashCandidates) {
        groupIdx++;
        event.sender.send("scan-progress", {
          scanId: "duplicates",
          progress: 70 + groupIdx / fullHashCandidates.length * 30,
          status: `Verifying group ${groupIdx}...`
        });
        const confirmedMap = /* @__PURE__ */ new Map();
        for (const filePath of paths) {
          try {
            const fileBuffer = await fs$1.readFile(filePath);
            const fullHash = crypto.createHash("md5").update(fileBuffer).digest("hex");
            if (!confirmedMap.has(fullHash)) {
              confirmedMap.set(fullHash, []);
            }
            confirmedMap.get(fullHash).push(filePath);
          } catch {
          }
        }
        for (const [hash, confirmedPaths] of confirmedMap) {
          if (confirmedPaths.length > 1) {
            const stats = await fs$1.stat(confirmedPaths[0]);
            finalGroups.push({
              hash,
              size: stats.size,
              files: confirmedPaths.map((p) => ({
                path: p,
      ...
      event.sender.send("scan-progress", { scanId: "duplicates", progress: 100, status: "Done" });
      return finalGroups.sort((a, b) => b.size - a.size);
    } catch (e) {
      console.error("Duplicate scan error", e);
      event.sender.send("scan-progress", { scanId: "duplicates", progress: 100, status: "Error" });
      return [];
    }
  });
  ipcMain$1.handle("get-store-value", (_event, key) => {
    return store.get(key);
  });
  ipcMain$1.handle("set-store-value", (_event, key, value) => {
    store.set(key, value);
  });
    ...
    ...isMac ? [
      {
        label: app$1.name,
        submenu: [
          { role: "about", label: `About ${app$1.name}` },
          { type: "separator" },
          { role: "services" },
          { type: "separator" },
          { role: "hide", label: `Hide ${app$1.name}` },
          { role: "hideOthers" },
          { role: "unhide" },
          { type: "separator" },
          { role: "quit", label: `Quit ${app$1.name}` }
        ]
      }
    ] : [],
    // { role: 'fileMenu' }
